...
Durable Object SQLite-backed virtual filesystem for Vibe Box.
...
- `SQLiteWorkspaceProvider` (the `@platformatic/vfs` adapter) implemented and exported from the package entrypoint; consumed by `@vibe-box/computerd`.
...
- Sync protocol building blocks implemented and exported; the typed RPC surface on top of them lives in `@vibe-box/computer-rpc`.
